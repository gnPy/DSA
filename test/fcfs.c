...
void statistics(Process *p , int n){
    float avg_tat = 0 , avg_wt = 0 , total_bt = 0, avg_rt = 0;
    for(int i = 0 ; i < n ; i++){
    ...
    float cpu_utilization = total_bt / p[n-1].completion_time ;
    float throughput = n / (float)p[n-1].completion_time ;
    ...
    for(int i = 0 ; i < n ; i++){
        if(i == 0){
            ...
            if(p[i].arrival_time > p[i-1].completion_time){
                //THIS IS WHEN THE PROCESS UNDERGOES IDLE TIME
                p[i].completion_time = p[i].arrival_time + p[i].burst_time ;
            }
            else{
                //THIS IS WHEN THE PROCESS DOES NOT UNDERGOES IDLE TIME
